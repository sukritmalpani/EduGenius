  ...
  try {
    ...
    const chapterPromises = chapters?.map(async (chapter, index) => {
      ...
      try {
        ...
        console.log(`YouTube API Response for ${query}:`, resp);

        const videoId =
          resp.length > index
            ? resp[index]?.id?.videoId
            : resp[Math.floor(Math.random() * resp.length)]?.id?.videoId;
        console.log(`Selected Video ID for Chapter ${index}:`, videoId);
